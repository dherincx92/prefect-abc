...
def _validate_build_output(build_method):
    ...
    Raises:
      - TypeError: if the output of `build` is not a prefect `flow` object
    """
    @wraps(build_method)
...
class AbstractFlow(ABC):
    ...
    def __new__(cls, *args, **kwargs):
        ...
        if cls.__name__ == 'AbstractFlow':
            raise NotImplementedError(f'''
              Abstract class `{cls.__name__}` should not be instantiated.
              Rather, it should be inherited by another class that implements
              a `build` method that returns a prefect `flow`
              '''
            )
    ...
    @abstractmethod
    def build(self):
        """Build method to be overwritten by user"""
        return None

    def run(self):
        """Runs a `prefect` flow"""

        # runs the flow itself; save flow in order to get its state below
        flow = self.build()

        # allows us to store the state of a flow and determine if it's
        # finished, pending, etc.
        state = flow.run()
        return state
